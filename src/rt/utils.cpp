#include "../include/rt.h"
	unsigned long long int utils::flen(FILE* file) {
		fseek(file, 0, SEEK_END);
		unsigned long long int len = ftell(file);
		fseek(file, 0, SEEK_SET);
		return len;
	}
	std::string utils::unix_path(std::string path) {
	...
	bool utils::string_replace(std::string& self, std::string _old, std::string _new) {
